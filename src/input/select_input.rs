...
use super::{
    ...
    input::InformType,
    select_list::{
        get_down_usize, get_up_usize, set_down_extend, set_left_area, set_left_area_ctrl,
        set_left_extend, set_right_area, set_right_area_ctrl, set_right_extend, set_up_extend,
    },
};
...
pub fn get_select_shift_informtype(
    key: Key,
    add_key: &mut Option<InformType>,
    is_pressed_ctrl: bool,
) {
    match key {
        Key::ArrowLeft => {
            *add_key = Some(InformType::SelectType(SelectType::Extend(
                Direction::Left,
                is_pressed_ctrl,
            )));
        }
...
pub fn set_select_text_list(
    key: &SelectType,
    text_list: &mut [String; 3],
    ...
    match key {
        ...
        SelectType::Down(is_ctrl) => {
            if *is_ctrl {
                ...
                let split_back =
                    text_list[2].split_chars_at(change_select_usize - text_list[0].size());
                ...
                Direction::Down => {
                    if text_field.select.is_open_right() || is_close {
                        ...
                        let list = if *is_ctrl {
                            ...
                            let change_select_usize =
                                get_down_usize(text_list, text_field.select.0);
                            text_list[1].split_chars_at(change_select_usize - text_list[0].size())
                        };
